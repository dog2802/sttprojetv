...
class TrayIcon:
    ...
    def set_loading_stage(self, message: str) -> None:
        """Обновляет только текст подсказки под текущим статусом - нужно для промежуточных
        этапов вроде "загружаю модель в память" после того, как скачивание уже завершилось
        (иначе трей молча остаётся на "скачиваю модель... 100%" и выглядит зависшим)."""
        self._icon.title = f"STTProjetV - {message}"
